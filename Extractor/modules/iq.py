...
async def handle_iq_logic(app, m):
    try:
        ...
        if logged_in:
            ...
            # Fetch courses
            json_master2 = await get_json("https://backend.studyiq.net/app-content-ws/api/v1/getAllPurchasedCourses?source=WEB", headers=headers)
            ...
            for batch_id in batch_ids:
                ...
                if batch_id:
                    master3 = await get_json(f"https://backend.studyiq.net/app-content-ws/v1/course/getDetails?courseId={batch_id}&languageId=", headers=headers)
                    ...
                    for t_id in content_id:
                        ...
                        parent_data = await get_json(f"https://backend.studyiq.net/app-content-ws/v1/course/getDetails?courseId={batch_id}&languageId=&parentId={t_id}", headers=headers)
                        ...
                        if all(sub_folder_order_id is None for sub_folder_order_id in subFolderOrderId):
                            ...
                            for p_id in content_idx:
                                ...
                                video = await get_json(f"https://backend.studyiq.net/app-content-ws/v1/course/getDetails?courseId={batch_id}&languageId=&parentId={t_id}/{p_id}", headers=headers)
                                for video_item in video['data']:
                                    url = video_item.get('videoUrl')
                                    name = video_item.get('name')
                                    if url is not None:
